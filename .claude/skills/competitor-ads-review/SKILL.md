...
`competitor-ads/inventory.json`. Para refrescar el inventario usar la tool MCP
...
- Corre el actor de Apify de Facebook Ad Library por cada página del
  inventario, guarda metadata en `competitor-ads/data/<ref>.json` y descarga
  los .mp4 a `competitor-ads/videos/<ref>/<ad_id>.mp4`.
...
```
Genera `competitor-ads/transcripts/<ref>/<ad_id>.txt`.

## Paso 3 — Extraer frames para visión
```bash
python3 competitor-ads/scripts/extract_frames.py
```
Genera 8 frames por video en `competitor-ads/frames/<ref>/<ad_id>/`.
